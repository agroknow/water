...
$('#' + map.mapId).data('gmap', gmap);
  ...
  if($.type(map.legend) !== "undefined"){
       ...
       */
      var oLegend = $('<div id="markers-legend">'+
                      '<h3>'+map.legend.header+'</h3>'+
                      '</div>');
      for(var i in map.legend.content){
        oLegend.append('<div data-gmap-marker-group="'+map.legend.content[i].text+'">'+
//                       '<input type="checkbox" value="'+map.legend.content[i].text+'"/>&nbsp;'+
                       '<img src="'+map.legend.content[i].iconUrl+'"/>&nbsp;'+
                       map.legend.content[i].text+
                       '</div>');      
      }
